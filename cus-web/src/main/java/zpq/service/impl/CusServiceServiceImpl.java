 ...
 * cusService服务层接口实现类
 * 
 * @author zcp 修改了数据库，添加了是否删除字段；删除操作不再是真正删除数据，而是修改该字段
 */
@Service("cusServiceService")
public class CusServiceServiceImpl implements ICusServiceService {
    @Autowired
    ...
    // return Msg.fail().add("msg", "添加失败：用户名【" + manager.getAccount() +
    // "】已存在");
    // }
     ...
     * Transaction Isolation Levels and Access Modes Read-only connections are
     * supported by the Oracle server, but not by the Oracle JDBC drivers.
     */
    ...
    public Msg addCustomer(Customer customer) {
        ...
        customerCriteria = customerExample.createCriteria();
        customerCriteria.andAccountEqualTo(customer.getAccount());
        List<Customer> customers1 = customerMapper.selectByExample(customerExample);
        if (!customers1.isEmpty()) {
            System.out.println(1);
            // return Msg.fail().add("msg", "添加失败：用户名【" + customer.getAccount()
            // + "】已存在");
        }
        // 测试不可重复度,MySQL默认隔离级别REPEATED-READ
        // 什么所谓的不可重读侧重修改，幻读侧重新增、删除都是胡说；
    ...
    public List<Customer> selectCustomer(String account, String onelevelAddress) {
        ...
        // 测试事务的readOnly
        List<Customer> customers2 = customerMapper.selectByExample(customerExample);
        return customers;
    }
