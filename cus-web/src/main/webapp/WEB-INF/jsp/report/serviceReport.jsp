		...
		<tr>
			<td>
				<select id="sel" style="float: right">
					<option value="product_fre">以产品分类(维修次数)</option>
					<option value="product_per">以产品分类(维修比例)</option>
...
<script>
	...
	function selDataByProduct(){
	    $.ajax({
		    url : "${ctx}/report/serviceReportByProd",
		    type : "GET",
		    success : function(result) {
				console.log(result);
				option.title.text = "售后统计-产品维修次数";
				...
				option.series[0].name = '申请维修次数';
				// 使用制定的配置项和数据显示图表
				myChart.setOption(option);
		    }
		});
	}
	/* 发送ajax请求，按产品分类获取维修比例记录 */
	function selDataByProductPer(){
	    $.ajax({
		    url : "${ctx}/report/serviceReportByProdPer",
		    type : "GET",
		    success : function(result) {
				console.log(result);
				option.title.text = "售后统计-产品维修比例";
				var xDataArr = new Array();
				var serDataArr = new Array();
	...
	function selDataByMonth(){
	    $.ajax({
		    url : "${ctx}/report/serviceReportByTime",
		    type : "GET",
		    success : function(result) {
				console.log(result);
				option.title.text = "售后统计-每月维修次数";
				var xDataArr = new Array();
				var serDataArr = new Array();
	...
	$('#sel').change(function(){
	    ...
	    if($('option:selected').val()=='product_fre'){
			selDataByProduct();
	    }
	    else if($('option:selected').val()=='month'){
			selDataByMonth();
	    }else{
			selDataByProductPer();
		}
